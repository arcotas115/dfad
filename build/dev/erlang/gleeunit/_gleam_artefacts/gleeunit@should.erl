...
-spec be_ok({ok, IBB} | {error, any()}) -> IBB.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, IBG}) -> IBG.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(IBJ)) -> IBJ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
