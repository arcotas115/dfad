...
-type action(GSO) :: stop | {continue, GSO, fun(() -> action(GSO))}.

-opaque yielder(GSP) :: {yielder, fun(() -> action(GSP))}.

-type step(GSQ, GSR) :: {next, GSQ, GSR} | done.

-type chunk(GSS, GST) :: {another_by,
        list(GSS),
        GST,
        GSS,
        fun(() -> action(GSS))} |
...
-type sized_chunk(GSU) :: {another, list(GSU), fun(() -> action(GSU))} |
    {last, list(GSU)} |
    no_more.
...
-spec unfold_loop(GTC, fun((GTC) -> step(GTD, GTC))) -> fun(() -> action(GTD)).
...
-spec unfold(GSX, fun((GSX) -> step(GSY, GSX))) -> yielder(GSY).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> GTH)) -> yielder(GTH).
repeatedly(F) ->
...
-spec from_list(list(GTL)) -> yielder(GTL).
from_list(List) ->
...
-spec transform(yielder(GTV), GTX, fun((GTX, GTV) -> step(GTY, GTX))) -> yielder(GTY).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(GUF)), fun((GUH, GUF) -> GUH), GUH) -> GUH.
...
-spec fold(yielder(GUC), GUE, fun((GUE, GUC) -> GUE)) -> GUE.
...
-spec to_list(yielder(GUK)) -> list(GUK).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(GUN)) -> step(GUN, yielder(GUN)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(GUV)), integer()) -> fun(() -> action(GUV)).
...
-spec take(yielder(GUS), integer()) -> yielder(GUS).
...
-spec drop_loop(fun(() -> action(GVB)), integer()) -> action(GVB).
...
-spec map(yielder(GVE), fun((GVE) -> GVG)) -> yielder(GVG).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(GVM), yielder(GVO), fun((GVM, GVO) -> GVQ)) -> yielder(GVQ).
...
-spec append_loop(fun(() -> action(GWC)), fun(() -> action(GWC))) -> action(GWC).
...
-spec append(yielder(GVY), yielder(GVY)) -> yielder(GVY).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(GWK)))) -> action(GWK).
...
-spec flatten(yielder(yielder(GWG))) -> yielder(GWG).
flatten(Yielder) ->
...
-spec concat(list(yielder(GWO))) -> yielder(GWO).
concat(Yielders) ->
...
-spec flat_map(yielder(GWS), fun((GWS) -> yielder(GWU))) -> yielder(GWU).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(GXA)), fun((GXA) -> boolean())) -> action(GXA).
...
-spec filter(yielder(GWX), fun((GWX) -> boolean())) -> yielder(GWX).
...
-spec filter_map(yielder(GXD), fun((GXD) -> {ok, GXF} | {error, any()})) -> yielder(GXF).
...
-spec cycle(yielder(GXR)) -> yielder(GXR).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(GXZ)), fun((GXZ) -> boolean())) -> {ok, GXZ} |
    {error, nil}.
...
-spec find(yielder(GXV), fun((GXV) -> boolean())) -> {ok, GXV} | {error, nil}.
...
-spec find_map(yielder(GYD), fun((GYD) -> {ok, GYF} | {error, any()})) -> {ok,
        GYF} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(GYW)), integer()) -> fun(() -> action({GYW,
    integer()})).
...
-spec index(yielder(GYT)) -> yielder({GYT, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(GYZ, fun((GYZ) -> GYZ)) -> yielder(GYZ).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(GZE)), fun((GZE) -> boolean())) -> fun(() -> action(GZE)).
...
-spec take_while(yielder(GZB), fun((GZB) -> boolean())) -> yielder(GZB).
...
-spec drop_while_loop(fun(() -> action(GZK)), fun((GZK) -> boolean())) -> action(GZK).
...
-spec drop_while(yielder(GZH), fun((GZH) -> boolean())) -> yielder(GZH).
...
-spec scan_loop(fun(() -> action(GZR)), fun((GZT, GZR) -> GZT), GZT) -> fun(() -> action(GZT)).
...
-spec scan(yielder(GZN), GZP, fun((GZP, GZN) -> GZP)) -> yielder(GZP).
...
-spec zip_loop(fun(() -> action(HAA)), fun(() -> action(HAC))) -> fun(() -> action({HAA,
    HAC})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(GZV), yielder(GZX)) -> yielder({GZV, GZX}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(HAP)), fun((HAP) -> HAR), HAR, list(HAP)) -> chunk(HAP, HAR).
...
-spec chunk_loop(fun(() -> action(HAK)), fun((HAK) -> HAM), HAM, HAK) -> action(list(HAK)).
...
-spec chunk(yielder(HAF), fun((HAF) -> any())) -> yielder(list(HAF)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(HBD)), integer(), list(HBD)) -> sized_chunk(HBD).
...
-spec sized_chunk_loop(fun(() -> action(HAZ)), integer()) -> fun(() -> action(list(HAZ))).
...
-spec sized_chunk(yielder(HAV), integer()) -> yielder(list(HAV)).
...
-spec intersperse_loop(fun(() -> action(HBK)), HBK) -> action(HBK).
...
-spec intersperse(yielder(HBH), HBH) -> yielder(HBH).
...
-spec any_loop(fun(() -> action(HBP)), fun((HBP) -> boolean())) -> boolean().
...
-spec any(yielder(HBN), fun((HBN) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(HBT)), fun((HBT) -> boolean())) -> boolean().
...
-spec all(yielder(HBR), fun((HBR) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(HCJ) -> fun((gleam@option:option(list(HCJ))) -> list(HCJ)).
...
-spec group_updater(fun((HCB) -> HCC)) -> fun((gleam@dict:dict(HCC, list(HCB)), HCB) -> gleam@dict:dict(HCC, list(HCB))).
group_updater(F) ->
...
-spec group(yielder(HBV), fun((HBV) -> HBX)) -> gleam@dict:dict(HBX, list(HBV)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(HCN), fun((HCN, HCN) -> HCN)) -> {ok, HCN} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(HCR)) -> {ok, HCR} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(HCZ) -> yielder(HCZ).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(HDF)), fun(() -> action(HDF))) -> action(HDF).
...
-spec interleave(yielder(HDB), yielder(HDB)) -> yielder(HDB).
...
-spec fold_until_loop(
    fun(() -> action(HDN)),
    fun((HDP, HDN) -> gleam@list:continue_or_stop(HDP)),
    HDP
) -> HDP.
...
-spec fold_until(
    yielder(HDJ),
    HDL,
    fun((HDL, HDJ) -> gleam@list:continue_or_stop(HDL))
) -> HDL.
...
-spec try_fold(yielder(HDR), HDT, fun((HDT, HDR) -> {ok, HDT} | {error, HDU})) -> {ok,
        HDT} |
    {error, HDU}.
...
-spec first(yielder(HEH)) -> {ok, HEH} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(HEL), integer()) -> {ok, HEL} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(HET), fun((HET) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(HEW, fun(() -> yielder(HEW))) -> yielder(HEW).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(HEZ), HEZ) -> yielder(HEZ).
