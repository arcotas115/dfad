...
-opaque supervisor(GDJ, GDK) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(GDJ, GDK))}.

-type message(GDL, GDM) :: any() | {gleam_phantom, GDL, GDM}.

-opaque builder(GDN, GDO) :: {builder,
        ...
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(GDN, GDO)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(GDP, GDQ) :: {local,
        gleam@erlang@process:name(message(GDP, GDQ))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(GDR) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, GDR}.
...
-type erlang_child_spec_property(GDS, GDT) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((GDS) -> {ok, gleam@otp@actor:started(GDT)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec start(builder(GFM, GFN)) -> {ok,
        gleam@otp@actor:started(supervisor(GFM, GFN))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec start_child(supervisor(GGS, GGT), GGS) -> {ok,
        gleam@otp@actor:started(GGT)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GHQ) -> {ok, gleam@otp@actor:started(GHR)} |
        {error, gleam@otp@actor:start_error()}),
    GHQ
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GHR, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((GEB) -> {ok, gleam@otp@actor:started(GEC)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GEB, GEC).
