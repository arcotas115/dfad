...
-opaque next(NKG, NKH) :: {continue,
        NKG,
        gleam@option:option(gleam@erlang@process:selector(NKH))} |
    normal_stop |
...
-opaque builder(NKI, NKJ) :: {builder,
        integer(),
        fun((gleam@http@request:request(NKI)) -> gleam@http@response:response(NKJ)),
...
-type websocket_message(NKK) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, NKK}.
...
-spec continue(NKL) -> next(NKL, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(NKP, NKQ), gleam@erlang@process:selector(NKQ)) -> next(NKP, NKQ).
...
-spec convert_next(next(NLE, NLF)) -> mist@internal@next:next(NLE, NLF).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(NMG)) -> gleam@http@response:response(NMI))
) -> builder(NMG, NMI).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(NMM, NMN), integer()) -> builder(NMM, NMN).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), NMS),
    integer(),
    gleam@http@response:response(NMS)
) -> builder(mist@internal@http:connection(), NMS).
...
-spec bind(builder(NNE, NNF), binary()) -> builder(NNE, NNF).
...
-spec with_ipv6(builder(NNK, NNL)) -> builder(NNK, NNL).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(NNQ, NNR), binary(), binary()) -> builder(NNQ, NNR).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(NOG)
) -> {ok, websocket_message(NOG)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((NOM, websocket_message(NON), mist@internal@websocket:websocket_connection()) -> next(NOM, NON)),
    fun((mist@internal@websocket:websocket_connection()) -> {NOM,
        gleam@option:option(gleam@erlang@process:selector(NON))}),
    fun((NOM) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(NPB)) -> {ok,
            gleam@otp@actor:initialised(NPD, NPB, any())} |
        {error, binary()}),
    fun((NPD, NPB, s_s_e_connection()) -> gleam@otp@actor:next(NPD, NPB))
) -> gleam@http@response:response(response_data()).
