...
-type valid_message(MVW) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, MVW}.

-type websocket_message(MVX) :: {valid, valid_message(MVX)} | invalid.
...
-type handler_message(MVY) :: {internal, gramps@websocket:frame()} | {user, MVY}.

-type websocket_state(MVZ) :: {websocket_state,
        bitstring(),
        MVZ,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(MXF))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(MXF))).
...
-spec apply_frames(
    ...
    fun((MWV, handler_message(MWW), websocket_connection()) -> mist@internal@next:next(MWV, MWW)),
    websocket_connection(),
    mist@internal@next:next(MWV, websocket_message(MWW)),
    fun((MWV) -> nil)
) -> mist@internal@next:next(MWV, websocket_message(MWW)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {MWI,
        gleam@option:option(gleam@erlang@process:selector(MWJ))}),
    fun((MWI) -> nil),
    fun((MWI, handler_message(MWJ), websocket_connection()) -> mist@internal@next:next(MWI, MWJ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(MWJ)))} |
    {error, nil}.
