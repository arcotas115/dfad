...
-type message(IQL) :: {internal, internal_message()} | {user, IQL}.

-type loop_message(IQM) :: {packet, bitstring()} | {custom, IQM}.

-type loop_state(IQN, IQO) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(IQO)),
        glisten@transport:transport(),
        IQN}.

-type connection(IQP) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(IQP))}.

-type next(IQQ, IQR) :: {continue,
        IQQ,
        gleam@option:option(gleam@erlang@process:selector(IQR))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(IQS, IQT) :: {handler,
        glisten@socket:socket(),
        fun((IQS, loop_message(IQT), connection(IQT)) -> next(IQS, loop_message(IQT))),
        fun((connection(IQT)) -> {IQS,
            gleam@option:option(gleam@erlang@process:selector(IQT))}),
        gleam@option:option(fun((IQS) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(IRG) -> next(IRG, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(IRK, IRL), gleam@erlang@process:selector(IRL)) -> next(IRK, IRL).
...
?DOC(false).
-spec start(handler(any(), ISA)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(ISA)))} |
