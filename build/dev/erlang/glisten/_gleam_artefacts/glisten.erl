...
-type message(JKY) :: {packet, bitstring()} | {user, JKY}.
...
-type connection(JKZ) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(JKZ))}.

-opaque next(JLA, JLB) :: {continue,
        JLA,
        gleam@option:option(gleam@erlang@process:selector(JLB))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(JLC, JLD) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(JLD)) -> {JLC,
            gleam@option:option(gleam@erlang@process:selector(JLD))}),
        fun((JLC, message(JLD), connection(JLD)) -> next(JLC, message(JLD))),
        gleam@option:option(fun((JLC) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(JMD) -> next(JMD, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(JMH, JMI), gleam@erlang@process:selector(JMI)) -> next(JMH, JMI).
...
?DOC(false).
-spec convert_next(next(JMW, JMX)) -> glisten@internal@handler:next(JMW, JMX).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(JNC, JND), fun((JND) -> JNG)) -> next(JNC, JNG).
...
-spec map_user_selector(gleam@erlang@process:selector(message(JNJ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(JNJ)).
...
-spec convert_loop(
    fun((JNO, message(JNP), connection(JNP)) -> next(JNO, message(JNP)))
) -> fun((JNO, glisten@internal@handler:loop_message(JNP), glisten@internal@handler:connection(JNP)) -> glisten@internal@handler:next(JNO, glisten@internal@handler:loop_message(JNP))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(JNU)) -> {JNW,
        gleam@option:option(gleam@erlang@process:selector(JNU))})
) -> fun((glisten@internal@handler:connection(JNU)) -> {JNW,
    gleam@option:option(gleam@erlang@process:selector(JNU))}).
...
-spec new(
    fun((connection(JOC)) -> {JOE,
        gleam@option:option(gleam@erlang@process:selector(JOC))}),
    fun((JOE, message(JOC), connection(JOC)) -> next(JOE, message(JOC)))
) -> builder(JOE, JOC).
new(On_init, Loop) ->
...
-spec with_close(builder(JOL, JOM), fun((JOL) -> nil)) -> builder(JOL, JOM).
...
-spec with_pool_size(builder(JOR, JOS), integer()) -> builder(JOR, JOS).
...
?DOC(false).
-spec with_http2(builder(JOX, JOY)) -> builder(JOX, JOY).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(JPJ, JPK)) -> builder(JPJ, JPK).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(JPP, JPQ), binary(), binary()) -> builder(JPP, JPQ).
...
-spec bind(builder(JPD, JPE), binary()) -> builder(JPD, JPE).
